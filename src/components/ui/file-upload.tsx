...
export const FileUpload = ({
    ...
    const handleFileChange = (newFiles: File[]) => {
        if (!isUploaded) {
            setFiles(newFiles);
            setIsUploaded(true); // Đánh dấu đã tải lên
            if (onChange) {
                onChange(newFiles);
            }
        }
                        ...
                        {!files.length && (
                            ...
                            >
                                {isDragActive ? (
                                    ...
                                    >
                                        Drop it
                                        <IconFileUpload className="h-4 w-4 text-neutral-600 dark:text-neutral-400" />
                                    </motion.p>
                                ) : (
                                    <IconFileUpload className="h-4 w-4 text-neutral-600 dark:text-neutral-300" />
                                )}
                            </motion.div>
                        )}
            ...
            {Array.from({ length: rows }).map((_, row) =>
                Array.from({ length: columns }).map((_, col) => {
                    ...
                    return (
                        <div
                            key={`${col}-${row}`}
                            className={`flex h-10 w-10 flex-shrink-0 rounded-[2px] ${index % 2 === 0
